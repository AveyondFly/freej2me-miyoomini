...
public class AndroidRecordStoreManager implements RecordStoreManager {
	...
	//数据存储路径
	private static String dataDir= Mobile.getPlatform().dataPath + "./rms/"+Mobile.getPlatform().loader.suitename;
	...
	public void deleteRecordStore(String recordStoreName) throws RecordStoreException {
		...
		recordStoreName = recordStoreName.replaceAll(ILLEGAL_FILENAME_CHARS, "");
				...
				for (String name : files) {
					if (name.startsWith(prefix) && name.endsWith(RECORD_STORE_RECORD_SUFFIX)) {
						...
						try (DataInputStream dis = new DataInputStream(new FileInputStream(file))) {
							...
							Log.e(TAG, "loadFromDisk: broken record " + file, e);
							...
							if (pLen + sLen < nLen) {
								try {
									...
									Log.d(TAG, "loadFromDisk: ERROR stubbing broken record " + file);
								}
							}
						}
					}
